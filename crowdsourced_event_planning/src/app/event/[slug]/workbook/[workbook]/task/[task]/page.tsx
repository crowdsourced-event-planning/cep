...
export async function generateMetadata({
  ...
  const { slug: eventSlug, workbook: workbookParam, task: taskParam } = params;
...
export default async function TaskDetailPage({ params }: TaskPageProps) {
  const { slug, workbook, task } = params;
  const [event, workbookData, taskData] = await Promise.all([
    getEventBySlug(slug),
    getWorkbookById(workbook),
    getTaskById(task),
  ]);
  if (!event || !workbookData || !taskData) {
    notFound();
  }
    ...
    <div className="min-h-screen bg-gray-50 py-8">
      <div className="container mx-auto px-4">
        ...
        <div className="flex items-center space-x-2 text-sm text-gray-500 mb-6">
          ...
          <span>/</span>
          <Link href={`/event/${event.slug}`} className="hover:text-blue-600">
            {event.title}
          </Link>
          <span>/</span>
          <Link
            href={`/event/${event.slug}/workbook/${workbookData._id}`}
            className="hover:text-blue-600"
          >
            {workbookData.name}
          </Link>
          <span>/</span>
          <span className="text-gray-900">{taskData.name}</span>
        </div>
              ...
              <div className="space-y-4">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-sm text-gray-600">
                  <div>
                    ...
                    {formatDateTime(taskData.createdAt || new Date())}
                  </div>
                  <div>
                ...
                <div className="space-y-3">
                  <div className="flex items-start space-x-3 p-3 bg-gray-50 rounded-lg">
                    ...
                    <div className="flex-1">
                      ...
                      <p className="text-gray-700 text-sm">Task was created</p>
                    </div>
                  </div>
                </div>
              </div>
