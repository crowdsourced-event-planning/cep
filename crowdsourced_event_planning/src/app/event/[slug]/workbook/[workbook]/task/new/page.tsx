...
export async function generateMetadata({
  ...
  const { slug: eventParam, workbook: workbookParam } = await params;
...
export default async function NewTaskPage({ params }: NewTaskPageProps) {
  const { slug, workbook } = await params;
  const [event, workbookData] = await Promise.all([
    getEventById(slug),
    getWorkbookById(workbook),
  ]);
  if (!event || !workbookData) {
    notFound();
  }
    ...
    <div className="min-h-screen bg-gray-50 py-8">
      <div className="container mx-auto px-4">
        ...
        <div className="flex items-center space-x-2 text-sm text-gray-500 mb-6">
          ...
          <span>/</span>
          <Link href={`/event/${slug}`} className="hover:text-blue-600">
            {event.title}
          </Link>
          <span>/</span>
          <Link
            href={`/event/${slug}/workbook/${workbook}`}
            ...
            <div className="space-y-6">
              <div>
                ...
                <p className="text-gray-600">
                  Add a new task to {workbookData.name}
                </p>
              </div>
